...
-- wrong scans and disqualification
alter table public.progress add column if not exists wrongs jsonb default '[]'::jsonb;
alter table public.progress add column if not exists disqualified_at timestamptz;
...
Scanning a room that is not a team's next stop is a **wrong scan**. When that happens:

- The buzzer sound plays on their phone, at full volume, and the phone vibrates.
- The screen says which wrong scan it was — "Wrong scan 2 of 3. One more and your team is out."
- **Three different wrong rooms disqualifies the team.** Their clues close, the scan form disappears,
  and every code they try from then on shows the disqualified screen.

Scanning the *same* wrong room twice only ever counts once, so a double tap on one poster cannot
knock a team out. Re-scanning a room they have already completed is not a wrong scan either — that
just re-issues the clue they are holding.

On the tracker each team shows **wrong scans as `n / 3`** with three dots, and the rooms they got
wrong are listed under the team name. A disqualified team turns red, drops to the bottom of the
table, and gets a **Reinstate** button — one tap clears their strikes and puts them back in, which
is worth knowing if a team is knocked out by accident.

Two notes on the sound. Phones only allow audio that a tap asked for, so it is unlocked silently the
moment a team presses the button and played once the answer is known. And a phone with its physical
silent switch on will not make noise whatever the page does — that is why it vibrates as well.
...
- **Tell teams the rule up front:** three wrong rooms and they are out. The buzzer is loud on purpose.
